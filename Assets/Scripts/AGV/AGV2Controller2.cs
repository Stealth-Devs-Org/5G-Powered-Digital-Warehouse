...
public class AGV4Controller : MonoBehaviour
{
    ...
    void Update()
    {
        if (webSocketClient != null && webSocketClient.agv4Message != null && webSocketClient.newmessageArrviedAGV4)
        {
            webSocketClient.newmessageArrviedAGV4 = false;
            agv4Message = webSocketClient.agv4Message;

            // Parse the JSON message
            AGVMessage message = JsonUtility.FromJson<AGVMessage>(agv4Message);

            Debug.Log($"AGV4 ID: {message.agv_id}");
            Debug.Log($"Location AGV4: X = {message.location[0]}, Y = {message.location[1]}");
            //Debug.Log($"Status AGV4: {message.status}");
            //Debug.Log($"Timestamp AGV4: {message.timestamp}");
