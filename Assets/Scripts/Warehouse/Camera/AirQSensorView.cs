...
public class AirQSensorView : MonoBehaviour
{
    private Camera mainCamera; // Reference to the main camera
    public float cameraSpeed = 5f; // Speed at which the camera moves to the target
    public List<GameObject> AirQSensors = new List<GameObject>(); 
    ...
    void Update()
    {
        ...
        if (AirQSensors.Count == 0)
        {
            return;
        }

        // Switch to the next sensor when '2' is pressed
        if (Input.GetKeyDown(KeyCode.Alpha4) && AirQSensors.Count > 0  && !Input.GetKeyDown(KeyCode.Alpha2) && !Input.GetKeyDown(KeyCode.Space) && !Input.GetKeyDown(KeyCode.Alpha3)&& !Input.GetKeyDown(KeyCode.Alpha5)&& !Input.GetKeyDown(KeyCode.Alpha1))
        {
            currentSensorIndex = (currentSensorIndex + 1) % AirQSensors.Count;
            ...
            currentFocusCoroutine = StartCoroutine(FocusOnSensor(AirQSensors[currentSensorIndex].transform));
        }


                
        if (Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.Alpha3)|| Input.GetKeyDown(KeyCode.Alpha5)|| Input.GetKeyDown(KeyCode.Alpha2)|| Input.GetKeyDown(KeyCode.Alpha1))
        {
    ...
    {
        GameObject[] allSensors = GameObject.FindGameObjectsWithTag("AirQualitySensor");

        foreach (GameObject sensor in allSensors)
        {
            if (!AirQSensors.Contains(sensor))
            {
                AirQSensors.Add(sensor);
            }
        }

        // Remove null entries (in case sensors are destroyed)
        AirQSensors.RemoveAll(sensor => sensor == null);
    }
    ...
    private void ShowSensorDetails(GameObject sensor)
    {
        ...
        if (sensorDetailPrefab != null)
        {
            ...
            currentSensorDetailUI.name = "SF WindowSensor AirQ";
        }
    }
